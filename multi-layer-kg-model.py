...
###############################################################################
'''
class SimpleMuliLayerHeteroGATPyG(nn.Module):
    """
    A heterogeneous multi-head GAT using PyTorch Geometric, extended to allow 
    multiple hidden layers (controlled by `num_layers`).

    Uses concat=True for each GATConv, so each layer's output dimension 
    is (hidden_dim * num_heads). For multiple layers, subsequent layers 
    must set in_channels = hidden_dim * num_heads.
    """
    ...
    def __init__(self, in_dim, hidden_dim, out_dim, num_heads=4, num_layers=2):
        ...
        ----------
        in_dim : int
            Dimension of the input node embeddings (e.g. 32).
        hidden_dim : int
            "out_channels" for each GAT layer (before multi-head concatenation).
        out_dim : int
            Final output dimension (e.g. 2 for binary classification).
        num_heads : int
        ...
        logging.info(f'[SimpleMuliLayerHeteroGATPyG] Using device: {self.device}')
        print(f'[SimpleMuliLayerHeteroGATPyG] Using device: {self.device}')
        ...
        # Linear projection from the raw embeddings to hidden_dim
        #  (only done once at the start)
        self.proj = nn.Linear(in_dim, hidden_dim)

        # We will build these HeteroConv-based layers lazily in forward() 
        #   so that we know the actual edge types from the data.
        self.hetero_layers = None

        # Output layer for 'user' node type.
        #   After the final GAT layer, dimension = hidden_dim * num_heads
    ...
    def forward(self, x_dict, edge_dict):
        ...
        out_feats : dict
            ...
            If 'user' is in out_feats, it will have shape [num_users, out_dim].
        """
        from torch_geometric.data import HeteroData

        # ----------------------------------------------------------------------
        # 1) Convert (x_dict, edge_dict) to a PyG HeteroData object
        # ----------------------------------------------------------------------
        data = HeteroData()
        for node_type, feats in x_dict.items():
            data[node_type].x = feats  # shape [num_nodes, in_dim]
        for (src_type, e_type, dst_type), edges in edge_dict.items():
            if len(edges) > 0:
                src, dst = zip(*edges)
            else:
                src, dst = [], []
            data[(src_type, dst_type)].edge_index = torch.tensor(
                [src, dst], dtype=torch.long
            )
        data = data.to(self.device)

        # ----------------------------------------------------------------------
        # 2) Project each node type from in_dim -> hidden_dim (only once)
        # ----------------------------------------------------------------------
        for node_type in data.node_types:
            data[node_type].x = self.proj(data[node_type].x)  
            # shape now [num_nodes, hidden_dim]

        # ----------------------------------------------------------------------
        # 3) Build the GAT layers lazily if not done yet
        #    The first layer uses in_channels=hidden_dim,
        #    subsequent layers use in_channels=hidden_dim * num_heads 
        #    because concat=True
        # ----------------------------------------------------------------------
        if self.hetero_layers is None:
            self.hetero_layers = nn.ModuleList()

            current_in = self.hidden_dim  # For the 1st layer
            for layer_idx in range(self.num_layers):
                ...
                for edge_type in data.edge_types:
                    conv_dict[edge_type] = GATConv(
                        ...
                        concat=True,          # => output shape = [hidden_dim * heads]
                        negative_slope=0.2,
                        dropout=0.3,
                        add_self_loops=False
                    )
                hetero_conv = HeteroConv(conv_dict, aggr='sum').to(self.device)
                self.hetero_layers.append(hetero_conv)

                # If concat=True, output dimension for each node is hidden_dim * num_heads
                current_in = self.hidden_dim * self.num_heads

        # ----------------------------------------------------------------------
        # 4) Forward pass through each GAT layer
        # ----------------------------------------------------------------------
        out_feats = {ntype: data[ntype].x for ntype in data.node_types}
        edge_index_dict = {etype: data[etype].edge_index for etype in data.edge_types}

        for layer_idx in range(self.num_layers):
            out_feats = self.hetero_layers[layer_idx](out_feats, edge_index_dict)
            # Apply ELU after each layer
            for ntype in out_feats:
                out_feats[ntype] = F.elu(out_feats[ntype])

        # ----------------------------------------------------------------------
        # 5) Final output layer for 'user' node type (if present)
        #    out_feats['user'] has shape [num_users, hidden_dim * num_heads],
        #    so we reduce it to [num_users, out_dim]
        # ----------------------------------------------------------------------
        if 'user' in out_feats:
            out_feats['user'] = self.out_layer(out_feats['user'])  # => [num_users, out_dim]
...
class SimpleMuliLayerHeteroGATPyG(nn.Module):
    ...
    def forward(self, x_dict, edge_dict):
        ...
        # 1. Convert input to PyG HeteroData object
        data = HeteroData()
        for node_type, feats in x_dict.items():
        ...
        # 3. Lazy initialization of GAT layers with residual connections
        if self.hetero_layers is None:
            self.hetero_layers = nn.ModuleList()
            current_in = self.hidden_dim
            for layer_idx in range(self.num_layers):
                conv_dict = {}
                for edge_type in data.edge_types:
                    conv_dict[edge_type] = GATConv(
                ...
                self.hetero_layers.append(HeteroConv(conv_dict, aggr='sum').to(self.device))
                current_in = self.hidden_dim * self.num_heads

        # 4. Forward pass with residual connections
        out_feats = {ntype: data[ntype].x for ntype in data.node_types}
        edge_index_dict = {etype: data[etype].edge_index for etype in data.edge_types}

        for layer_idx, layer in enumerate(self.hetero_layers):
            updated_feats = layer(out_feats, edge_index_dict)

            # Apply residual connection: updated_feats += original_feats
            for ntype in updated_feats:
                if updated_feats[ntype].shape == out_feats[ntype].shape:
                    updated_feats[ntype] += out_feats[ntype]  # Residual connection
                out_feats[ntype] = F.elu(updated_feats[ntype])  # Apply activation

        # 5. Output layer for 'user' node type
        if 'user' in out_feats:
...
def train_in_chunks(
    ...
    model = SimpleMuliLayerHeteroGATPyG(
